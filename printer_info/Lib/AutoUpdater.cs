...
{
    /// <summary>
    /// Auto updater for printer_info service
    /// </summary>
    public class AutoUpdater
    {
        ...
        /// Uses /api/service/version endpoint for printer_info service (RESTful GET)
        /// </summary>
        ...
        public async Task<bool> DownloadAndInstallAsync(UpdateInfo info)
        {
            ...
            var tempPath = Path.Combine(Path.GetTempPath(), $"printer_info_update_{uniqueId}");
            ...
            try
            {
                ...
                var arguments = $"\"{installPathArg}\" \"{tempPathArg}\" \"printer_info\"";
        ...
        private void CleanupOldTempFolders()
        {
            try
            {
                ...
                foreach (var dir in Directory.GetDirectories(tempRoot, "printer_info_update_*"))
                {
                    try
                    {
